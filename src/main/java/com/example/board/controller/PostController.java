...
public class PostController {
    ...
    //들어오는 요청값 검사하기 위해 @Valid
    //파일첨부를 위해 form-data 형식 사용
    public ResponseEntity<Post> save (@Valid @RequestPart("post") PostSaveDto postSaveDto, @RequestPart("files") List<MultipartFile> files){
        log.info("[Controller] 게시글 저장 요청 들어옴");
        Post requestPost = postService.save(postSaveDto, files); //postService에서 return 해준 덕분
